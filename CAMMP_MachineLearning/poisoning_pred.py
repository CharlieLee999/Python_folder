...
activ_func = ["tanh", "tanh", "relu"]
batch_size = 6
epochs= 500
epochs_step = int(epochs/50)
num_test = 5

flag_plot_tmp_contour = True
flag_normlize = True
flag_add_full_one = True
...
#    model.add(Dropout(0.2))
    ...
    ### Second hidden layer ###
    model.add(Dense(2*dim_input, kernel_initializer='glorot_uniform'))
#    model.add(BatchNormalization())
    model.add(Activation(activ_func[1]))
##    
    
    ### Third hidden layer ###
    model.add(Dense(3*dim_input,  kernel_initializer='glorot_uniform'))
#    model.add(BatchNormalization())
...
def getLabelColorInGrid(X_orig, scaler, model, flag_normlize=True, flag_add_full_one=True):
    ...
    color_sim = ['white' if i==1 else 'grey' for i in Y_sim_pred ]
...
#plt.savefig('Original_data.png', format='png')
#plt.show()
...
#plt.savefig('Normalized_data.png', format='png')
#plt.show()
...
#            plt.hold(True) # Hold on
        plt.scatter(X_grid_inverse[:,0], X_grid_inverse[:,1], c= color_sim, s= 20, marker='s')
...
plt.savefig(contour_dir+"/final_simulated_contour.png", format='png')
plt.show()
...
if flag_plot_tmp_contour == False:
    ...
    plt.savefig(acc_loss_dir+"/accuracy_aver.png", format='png')
    ...
    plt.grid()
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig(acc_loss_dir+"/loss_aver.png", format='png')
